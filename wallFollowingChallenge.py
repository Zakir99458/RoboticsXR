...
frontSensorS01 = robot.getDistanceSensor("so1")
frontSensorS01.enable(timestep)
sideSensorS02 = robot.getDistanceSensor("so2")
sideSensorS02.enable(timestep)
sideSensorS07 = robot.getDistanceSensor("so7")
sideSensorS07.enable(timestep)
...
while robot.step(timestep) != -1:
    #
    if getDistance(frontSensorS01) < 0.3 or getDistance(sideSensorS02) < 0.3 or getDistance(frontSensor) < 0.3:
        leftWheel.setVelocity(MAX_SPEED)
        rightWheel.setVelocity(-MAX_SPEED)
    ...
    elif getDistance(sideSensor) > 0.35:
